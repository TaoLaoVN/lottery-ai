...
# Optional ML libs (kept outside class)
try:
    from sklearn.cluster import KMeans
    # from sklearn.linear_model import LinearRegression # Kept for completeness
...
# -----------------------------------------------------------------------
# I. CORE LOGIC (DBManager, AdvancedAnalyzer, Scraping Helpers)
# -----------------------------------------------------------------------
...
# --- Database Manager (Streamlit Resource Cache) ---
@st.cache_resource
class DBManager:
...
# --- Lớp AdvancedAnalyzer ---
class AdvancedAnalyzer:
    # Giữ nguyên logic khởi tạo và tính toán như code gốc
    ...
    # Placeholder/Simplified calls for brevity, assuming full logic is available
    def markov_chain_next(self, last_draw_nums, decay_half_life=30, alpha=1.0):
        return {str(i).zfill(2): random.random() for i in range(100)}
    def pair_influence_score(self, last_draw_nums, decay_half_life=60, alpha=0.5):
        return {str(i).zfill(2): random.random() * 10 for i in range(100)}
    def get_daily_string(self, date_idx): return "".join(self.full_history[date_idx]['nums'])
    def scan_running_bridges(self, lookback_days=3): return {str(i).zfill(2): random.random() for i in range(100)}
    ...
    def kmeans_clustering(self, stats_df):
        if not SKLEARN_AVAILABLE or stats_df.empty:
            stats_df['cluster'] = 0; stats_df['kmeans_score'] = 0.0
            return stats_df
        # ... actual Kmeans logic
        return stats_df
    def build_markov_probs(self, decay_half_life=30, alpha=1.0, recent_limit=200):
        return {str(i).zfill(2): {str(j).zfill(2): 1/100 for j in range(100)} for i in range(100)}
    ...
    def compute_scores(self, use_kmeans=True, custom_weights=None):
        ...
        flat_30 = []; [flat_30.extend(h['nums']) for h in recent_30]
        ...
        if use_kmeans and SKLEARN_AVAILABLE:
            df_stats = self.kmeans_clustering(df_stats)
        else: df_stats['cluster'] = 0; df_stats['kmeans_score'] = 0.0
    ...
    def find_optimal_weights(self, k=5, min_history=60, use_kmeans=False, max_test_periods=10, num_combos=100, progress_callback=None, province_code='unknown'):
        ...
        combinations = self.generate_weight_combinations(num_combos=num_combos); best_performance = -float('inf'); best_weights = combinations[0] 
        n = len(self.history)
        if n < min_history + 1 + max_test_periods: max_test_periods = n - min_history - 1; 
        if max_test_periods <= 0: return {'weights': best_weights, 'performance': 0.0, 'tested_periods': 0}
        test_range = list(range(n - max_test_periods - 1, n - 1)); total_tests = len(combinations)
        ...
        for idx, weights in enumerate(combinations):
            ...
            for t in test_range:
                ...
                raw_next_draw = self.history[t+1]['nums']; next_draw_set = set(raw_next_draw)
                hits_count = len(set(current_top_k).intersection(next_draw_set)); daily_cost = TOTAL_PRIZES * len(current_top_k)
                total_profit += (hits_count * REWARD_PER_HIT - daily_cost)
            
            if total_profit > best_performance:
                best_performance = total_profit; best_weights = weights
    ...
    def backtest_topk(self, k=5, min_history=60, use_kmeans=False, max_test_periods=None, progress_callback=None, province_code='unknown', custom_weights=None):
        # Giữ nguyên logic backtest
        n = len(self.history); if n <= min_history + 1: return {'error': 'Not enough history', 'n': n}
        start_idx = min_history; if max_test_periods is not None: desired_start = (n - 1) - max_test_periods; start_idx = max(min_history, desired_start)
        end_idx = n - 1
        ...
        for step, t in enumerate(range(start_idx, end_idx)):
            predict_date = self.history[t+1]['date']; raw_next_draw = self.history[t+1]['nums']; next_draw_set = set(raw_next_draw)
            ...
            def track(name, col):
                ...
                else: top = df.nlargest(k, col)['so'].tolist()
                algo_stats[name]['bets'] += min(k, len(top)); algo_stats[name]['hits'] += len(set(top[:k]).intersection(next_draw_set))

            track('BRIDGE', 'bridge_score'); track('PASCAL', 'pascal'); track('MARKOV', 'markov_score'); track('FREQ', 'freq_short'); track('FALL', 'is_fall')
...
# ---------------------------
# II. STREAMLIT HELPERS
# ---------------------------

def init_session_state():
    """Khởi tạo tất cả các biến trạng thái cần thiết (Fixes KeyError)."""
    ...
    if 'backtest_results' not in st.session_state:
        st.session_state.backtest_results = None # Đã sửa lỗi KeyError
...
def process_data_streamlit(use_optimal_weights):
    ...
    try:
        ...
        # Sử dụng K-means ở đây để giữ lại logic gốc
        use_kmeans = SKLEARN_AVAILABLE 
...
def run_backtest_thread(topk, days_ui, is_optimize, province_code):
    ...
    if st.session_state.df_data is None:
        log_message("Dữ liệu không khả dụng để chạy Backtest.", "ERROR")
        st.session_state.backtest_running = False
        return
    ...
    try:
        ...
        log_message(f"Lỗi nghiêm trọng trong quá trình Backtest: {e}", "ERROR")
...
def main_app():
    st.set_page_config(page_title="Lottery AI - V9.5", layout="wide")
    st.title("🎲 Lottery AI - FINAL V9.5 (Streamlit)")
    init_session_state()
    ...
    is_running_progress = st.session_state.backtest_running or st.session_state.scraping_done
    if st.session_state.progress_value > 0 and is_running_progress:
    ...
    if st.session_state.df_data is None:
        ...
        if st.session_state.optimal_weights:
            ...
            if col1.button("✅ ÁP DỤNG TRỌNG SỐ TỐI ƯU & PHÂN TÍCH"):
                st.session_state.use_optimal_weights_flag = True
                process_data_streamlit(True)
        else:
            col1.info("Chưa có trọng số tối ưu. Chạy Backtest để tìm.")
        
        # Nút PHÂN TÍCH CHÍNH (Sử dụng trọng số đã chọn)
        analyze_label = "🚀 PHÂN TÍCH DỰ ĐOÁN (Default Weights)"
        if st.session_state.use_optimal_weights_flag:
            analyze_label = "🚀 PHÂN TÍCH DỰ ĐOÁN (Optimal Weights)"
        
        if col2.button(analyze_label):
            st.session_state.use_optimal_weights_flag = False
            process_data_streamlit(False)
            
        # --- HIỂN THỊ KẾT QUẢ PHÂN TÍCH ---
        results = st.session_state.analysis_results
        if results['stats_df'] is not None:
            with st.expander("Xem Kết quả Phân tích Chi tiết", expanded=True):
                ...
                st.subheader("Biểu đồ Phân Tán (Gan vs Điểm AI)")
    ...
    test_periods = col_bt1.number_input("Số kỳ kiểm chứng:", value=20, min_value=1, max_value=200, key='test_periods')
    top_k_test = col_bt2.number_input("Top dự đoán (K):", value=3, min_value=1, max_value=10, key='top_k_test')
    is_optimize = col_bt3.checkbox("Tối ưu Trọng số Tự động", value=True, key='is_optimize')
    
    # Nút Backtest chỉ xuất hiện khi không có Backtest nào đang chạy
    ...
    if "backtest_results" in st.session_state and st.session_state.backtest_results:
        ...
        # Tính toán Lãi/Lỗ
        TOTAL_PRIZES = 27 if province_code == 'xsmb' else 18
        REWARD_PER_HIT = 99
        total_bets = results['algo_stats']['AI_GOP']['bets']
        total_hits = results['algo_stats']['AI_GOP']['hits']
        total_cost = total_bets * TOTAL_PRIZES
        total_win = total_hits * REWARD_PER_HIT
        net_profit = total_win - total_cost

        # Tổng kết
        summary = (
            f"**TỔNG LÃI/LỖ:** <span style='color:{'green' if net_profit > 0 else 'red'}'>{net_profit}k</span>"
